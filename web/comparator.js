...
(function () {
  ...
  function init() {
    ...
    if (!root) return;
    // Styles + a loading placeholder go up immediately, before the data
    // fetch even starts - otherwise #rt-hoius sits completely empty (no
    // height, unstyled) until all three requests resolve, then the whole
    // widget pops in at once. Filling it in instead of popping in reads as
    // much smoother, independent of how fast the fetch itself is.
    injectStyles();
    root.className = 'rt-comparator';
    root.innerHTML = '<div class="rt-loading"><span class="rt-loading__spinner" aria-hidden="true"></span>Laen hoiuste andmeid...</div>';
