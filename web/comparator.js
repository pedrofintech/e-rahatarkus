...
(function () {
  ...
  function positionTipBubble(tipEl) {
    ...
    if (sidebar) {
      ...
      var maxW = Math.max(160, sr.width - 16);
      bubble.style.maxWidth = Math.min(maxW, 352) + 'px';
      bw = Math.min(bw, maxW);
      var minLeft = sr.left + 8, maxLeft = sr.right - 8 - bw;
      var left = Math.max(minLeft, Math.min(r.left + r.width / 2 - bw / 2, Math.max(minLeft, maxLeft)));
      var below = r.top - gap - bh < 8;
      ...
      if (arrow) {
        var margin = 18;
        var arrowLeft = Math.max(margin, Math.min(bw - margin, r.left + r.width / 2 - left));
    ...
    var flipLeft = r.right + gap + bw > window.innerWidth - 8;
